import { AfterContentInit, Component, ContentChildren, ElementRef, 
  EventEmitter, forwardRef, Input, HostListener, OnDestroy, Optional, 
  Output, Renderer, QueryList, ViewEncapsulation, Directive } from '@angular/core';
...
@Component({
  ...
  template:
    ...
    '</button>',
  host: {
    '[class.bdq-select-disabled]': '_disabled'
  },
  providers: [SELECT_VALUE_ACCESSOR],
  encapsulation: ViewEncapsulation.None,
})
export class BdqSelect extends Ion implements AfterContentInit, ControlValueAccessor, OnDestroy {
  _disabled: any = false;
  _labelId: string;
  ...
  constructor(
    private _globalNavLocker: GlobalNavLocker,
    private _app: App,
    private _form: Form,
    config: Config,
  ...
  open(){
    ...
    console.debug('bdq-select, open alert');
    ...
    if (this.interface === 'action-sheet' && this._multi) {
      console.warn('Interface cannot be "action-sheet" with a multi-value bdq-select. Using the "alert" interface.');
      this.interface = 'alert';
    }

    let overlayPromise: any;
    if (this.interface === 'action-sheet') {
      ...
      overlayPromise = this._globalNavLocker.tryPresentActionSheet(selectOptions);
